...
#define SIZE 64 
using namespace std;
void DES_OFB(int *plaintext_original,int *ciphertext,char *p,int *c);
...
{	
	...
	fp = fopen("Picture.bmp", "rb");  
 	out = fopen("Encryption.bmp", "wb");  
 	if (fp == NULL) {
	...
	if(i%8!=7){
		...
		DES_OFB(iv,eniv,image_sc,image_ti);
...
/*		if(i==7){
		...
		for(int ss=0;ss<64;ss++){
			intimage_sc[ss]=(int)image_sc[ss] - '0';
		}
		for(int ss=0;ss<64;ss++){
			image_t[ss]=intimage_sc[ss] xor eniv[ss];
		}
*/
		image_sp=0;
