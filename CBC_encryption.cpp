...
#define SIZE 64 
using namespace std;
void DES_CBC(char *plaintext_original,int *ciphertext,int *iv);
...
void DES_CBC(char *plaintext_original,int *ciphertext,int *iv){
	...
	for(int i = 0; i < SIZE; i++){
		IV[i]=iv[i] xor plaintext[i];
	}
 	//Plaintext do after Initial Permutation 
	for(int i = 0; i < SIZE; i++){
		IPtext[i]=IV[InitialPermutation[i]-1];
	}
	...
	for(int i=0;i<64;i++){
		...
		iv[i]=FPtext[FinalPermutation[i]-1];
	}
